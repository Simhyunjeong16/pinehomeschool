...
class sendDB(QObject):
    ...
    def insert_info_db(self):
        count1 = 0
        while (not emotionQ.empty()):  # 60초만 돌도록 바꿔야됨..
...
############################################################################################################
    ...
    def A_showMessageBox(self):
        ...
        ################################ 보상체계 DB, A:1, B:2, C:3
        # starcount1 = 1
        # connection4 = pymysql.connect(host='203.252.208.222', user='root', password='MindTree@12345',
        #                               db='Compensation_system', port=8008)
        # with connection4.cursor() as cursor:
        #     sql = 'INSERT INTO Compensation_system (value) VALUES ( %s )'
        #     cursor.execute(sql, (starcount1))
        #     connection4.commit()
        #     connection4.close()
        ################################
    ...
    def B_showMessageBox(self):
        ...
        ################################ 보상체계 DB, A:1, B:2, C:3
        # starcount2 = 2
        # connection5 = pymysql.connect(host='203.252.208.222', user='root', password='MindTree@12345',
        #                               db='Compensation_system', port=8008)
        # with connection5.cursor() as cursor:
        #     sql = 'INSERT INTO Compensation_system (value) VALUES ( %s )'
        #     cursor.execute(sql, (starcount2))
        #     connection5.commit()
        #     connection5.close()
        ################################
    ...
    def Star_showMessageBox_c(self):  #####################보상체계 메세지박스
        msg = QMessageBox()
        msg.setWindowTitle("보상")
        msg.setStyleSheet("background-color: rgb(253, 225, 120);")
        msg.setWindowIcon(QIcon("mainlogo.png"))
    ...
    def ffinish(self):
        ...
        connection3 = pymysql.connect(host='203.252.208.222', user='root', password='MindTree@12345', db='emotion_sum',
                                      port=8008)
        ...
        if endqueue.get() == 1:
            if (average > emotionvalue[0] - 20 and average < emotionvalue[0] + 20):
                print("부모감정", emotionvalue[0] - 20, "~", emotionvalue[0]+ 20, ", 아이평균감정 : ", average)
                self.A_showMessageBox()

            elif (average > emotionvalue[0] - 40 and average < emotionvalue[0]+ 40):
                print("부모감정", emotionvalue[0] - 40, "~", emotionvalue[0] + 40, ", 아이평균감정 : ", average)
