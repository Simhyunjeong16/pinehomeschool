...
class EyeTribe():
    ...
    def connect(self):
        ...
        if self._sock is None:
            ...
            try:
                ...
                self._hbinterval = int(p['values']['heartbeatinterval']) / 1000.0 / 30000 # 이거 99.99로 나오는데 0.033으로 나오도록 해야함
    ...
    def mysql(self):
        ...
        # print("frame:", EyeTribe.Frame( self._tell_tracker(EyeTribe.etm_get_frame)['values']['frame']))

        conn = pymysql.connect(host='203.252.208.222', user='root', password='MindTree@12345', db='eyetribe', port=8008)
        ...
        try:
            with conn.cursor() as cursor:
                sql = 'INSERT INTO coord (time_now,Fix,State,Rwx,Rwy,Avx,Avy,LRwx,LRwy,LAvx,LAvy,LPSz,LCx,LCy,RRwx,RRwy,RAvx,RAvy,RPSz,RCx,RCy)VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
                cursor.execute(sql,(nowTime,splt_n[3],splt_n[4],splt_n[5],splt_n[6],splt_n[7],splt_n[8],splt_n[9],splt_n[10],splt_n[11],splt_n[12],splt_n[13],splt_n[14],splt_n[15],splt_n[16],splt_n[17],splt_n[18],splt_n[19],splt_n[20],splt_n[21],splt_n[22]))
